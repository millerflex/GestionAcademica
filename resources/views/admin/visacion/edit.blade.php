...
<div class="card">
    <div class="card-header bg-warning text-white">
        <h3 class="card-title"><i class="fas fa-edit"></i> Editar Silabo</h3>
    </div>
    <div class="card-body">
        <form method="POST" action="">
            {{-- <form method="POST" action="{{ route('admin.visacion.update', $silabo->id) }}"> --}}
            @method('PUT')
            @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="curso_id" name="curso_id">
                            @foreach($cursos as $curso)
                                <option value="{{ $curso->id }}" {{ $curso->id == $silabo->curso_id ? 'selected' : '' }}>
                                    {{ $curso->nombre }}
                                </option>
                            @endforeach
                        </select>
